...
export default class Client {
  ...
  verifyImpersonation({request, token, name, metadata}, done) {
    if (request && !token) {
      try {
  ...
  verify({request, token, name, metadata}, done) {
    return this.promiseProxy(done, {request, token, name, metadata}, this.verifyImpersonation.bind(this));
  }
}
